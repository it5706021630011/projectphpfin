<?php
    ...
    if(isset($_POST['add_item'])){
        ...
        $po_amount = intval($_POST['pro_amount']);
        $pro_price = intval($_POST['pro_price']);
            ...
            <div class="col-xs-3">
                <div class="box">
                    <div class="box-header">
                        ...
                        <div class="box-body">
                            <?php
                                $sql_so = "SELECT * FROM sale_order JOIN sale_order_detail ON sale_order.so_id = sale_order_detail.so_id JOIN employee ON sale_order.emp_id = employee.emp_id JOIN sale_order_type ON sale_order_type.so_type_id = sale_order.so_type_id JOIN sales_status.sal_stu_id = sale_order.sal_stu_id AND sale_order.so_id = '".$_GET["id"]."'";
                                $query_so = mysqli_query($conn,$sql_so);

                                while($row = mysqli_fetch_array($query_so,MYSQLI_ASSOC))
                                {
                            ?>
                            <div class="form-group">
                                <label>รหัสการขายสินค้า</label>
                                <input type="text" class="form-control" id="po_id" value="<?php echo "SO-".sprintf("%04d",$row['po_id']);?>" readonly>
                                <input type="hidden" id="new_po_id" value="<?php echo $row['so_id'];?>">
                            </div>
                            <div class="form-group">
                                ...
                                <?php
                                    if(!$edit_so_ck){
                                ?>
                                    <input type="date" class="form-control" id="new_po_date" value="<?php echo $row['so_date']?>" readonly>
                                <?php
                                    }
                                    else{
                                ?>
                                    <input type="date" class="form-control" id="new_po_date" value="<?php echo $row['so_date']?>">
                                <?php
                                    }
                                ?>
                            </div>
